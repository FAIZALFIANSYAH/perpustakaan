...
export default function Index({ users, filters }: Props) {
    ...
    const isSuperAdmin =
        currentUser?.roles?.some((r) => r.name === 'Super Admin') ?? false;
                            ...
                            {users.length > 0 ? (
                                users.map((user) => (
                                    <tr key={user.id}>
                                        ...
                                        <td className="p-4">
                                            <span
                                                className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${user.is_online
                                                        ? 'bg-emerald-100 text-emerald-700'
                                                        : 'bg-slate-100 text-slate-600'
                                                    }`}
                                            >
                                                <span
                                                    className={`inline-block h-1.5 w-1.5 rounded-full ${user.is_online
                                                            ? 'bg-emerald-500'
                                                            : 'bg-slate-400'
                                                        }`}
                                                />
